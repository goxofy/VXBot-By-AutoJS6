...
一个基于 AutoJS6 的 VX 自动化机器人框架。支持自动回复、OpenAI 对话、图片发送、会话管理等功能。

> **⚠️ 免责声明**: 本项目仅供学习与技术研究，请勿用于发送垃圾信息或骚扰他人。使用者需自行承担风险，使用不当可能导致 VX 账号被限制。
...
*   **📡 主动轮询**: 列表轮询机制，主动扫描未读消息，解决新版 VX 不弹通知的问题。
*   **🛡 智能过滤**: 
    *   **白名单机制**: 只回复指定好友/群的会话。
...
bot.start({
    ...
    whitelist: WHITELIST,
    mentionString: "@Bot" // 群聊触发关键词 (私聊默认通过)
});
```
